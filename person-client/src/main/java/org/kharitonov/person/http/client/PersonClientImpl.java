...
public class PersonClientImpl implements PersonClient{
    ...
    public CustomPageImpl<PersonDTO> findAllPerson(int page, int size) {
        HttpRequest request = createGetRequest(page, size);
        String response = sendRequest(request);
        return deserializeList(response);
    }

    public PersonDTO getPerson(String name) {
        HttpRequest request = createGetRequest(name);
        String response = sendRequest(request);
    ...
    public String addPerson(String name, int age) throws JsonProcessingException {
        ...
        HttpRequest request = createPostRequest(BASE_URI + "/persons", personAsString);
    ...
    public HttpRequest createGetRequest(String name) {
        try {
            return HttpRequest.newBuilder()
                    .uri(new URI(BASE_URI + "/persons/" + name))
                    .GET()
                    .build();
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
    ...
    public HttpRequest createGetRequest(int page, int size) {
        try {
            return HttpRequest.newBuilder()
                    .uri(new URI(BASE_URI + "/persons?page=" + page + "&size=" + size))
                    .GET()
                    .build();
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
