...
public class ControllerUnitTest extends AbstractIntegrationServiceTest {
    ...
    @Test
    public void getByNonExistedNamePersonControllerTest() throws Exception {
        String randomName = RandomStringUtils.randomAlphabetic(12);
        this.mockMvc
                .perform(get("/persons/" + randomName))
                .andDo(print()).andExpect(status().isNotFound())
                .andExpect(result -> Assertions.assertTrue
                        (result.getResolvedException() instanceof PersonNotFoundException))
                .andExpect(jsonPath("$.message").value("Could not find person with name - " + randomName));
    }
